...
print(f"scikit-learn 版本号: {sklearn_version}")
...
# Function to predict the target for a given dataset
def predict_batch_data(data):
    # Preprocess the data
    ...
    disease_percentage = y_pred.mean()

    # Add a new column to the data indicating whether each sample has heart disease or not
    data['predicted_target'] = y_pred

    return disease_percentage, data

# Input the CSV file path from the console
csv_file_path = input("Please enter the CSV file path: ")

# Read the batch data from the CSV file
# Read the batch data from the CSV file
import os

# Read the batch data from the CSV file
batch_data = pd.read_csv(csv_file_path, header=None, encoding='utf-8')
batch_data.columns = df.columns[:-1]

# Preprocess the batch data
batch_data['sex'] = batch_data.sex.map({'female': 0, 'male': 1})
batch_data['thal'] = batch_data.thal.fillna(batch_data.thal.mean())
batch_data['ca'] = batch_data.ca.fillna(batch_data.ca.mean())

X_test = batch_data.values
X_test_scaled = scaler.transform(X_test)
...
batch_data['predicted_target'] = y_pred

# Save the prediction results to a new CSV file
output_file_path = os.path.join(os.path.dirname(csv_file_path), os.path.basename(csv_file_path).replace('.csv', '_result.csv'))
batch_data.to_csv(output_file_path, index=False)

# Print the percentage of patients with heart disease
print("Percentage of patients with heart disease in the batch data: {:.2%}".format(disease_percentage))
