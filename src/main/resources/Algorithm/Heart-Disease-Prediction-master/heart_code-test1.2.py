...
# 解释预测结果
def explain_prediction(instance, model, feature_names):
    ...
    # 找到贡献最大的特征
    max_contribution_feature = max(contributions, key=contributions.get)
    ...
    # 输出贡献最大的特征
    print("贡献最大的参数：", max_contribution_feature)
...
# 解释预测结果
explain_prediction(instance, xg, df.columns[:-1])